...
namespace updater.software
{
    ...
    public class Audacity : NoPreUpdateProcessSoftware
    {
        ...
        {
            logger.Info("Searching for newer version of Audacity...");
            string htmlCode = null;
            using (var client = new HttpClient())
            {
                try
                {
                    var task = client.GetStringAsync("https://www.fosshub.com/Audacity.html");
                    task.Wait();
            ...
            var reVersion = new Regex("audacity\\-win\\-([0-9]+\\.[0-9]+\\.[0-9]+)\\.exe");
            ...
            var reChecksum = new Regex("audacity\\-win\\-"+Regex.Escape(version)+"\\.exe\",\"r\":\"[0-9a-f]+\",\"hash\":\\{\"md5\":\"[0-9a-f]{32}\",\"sha1\":\"[0-9a-f]{40}\",\"sha256\":\"([0-9a-f]{64})\"");
