...
{
    /// <summary>
    /// Handles updates of Eclipse Temurin JRE 25 with Hotspot JVM.
    /// </summary>
    public class OpenJRE25 : AbstractSoftware
    {
        /// <summary>
        /// NLog.Logger for OpenJRE21 class
        /// </summary>
        private static readonly NLog.Logger logger = NLog.LogManager.GetLogger(typeof(OpenJRE25).FullName);
        ...
        public OpenJRE25(bool autoGetNewer)
            : base(autoGetNewer)
        { }
        ...
        public override AvailableSoftware knownInfo()
        {
            ...
            const string version = "25.0.0.36";
            return new AvailableSoftware("Eclipse Temurin JRE 25 with Hotspot",
                version,
                null, // no 32-bit installer
                "^Eclipse Temurin JRE [a-z]+ Hotspot 25\\+[0-9]+ \\(x64\\)$",
                null, // no 32-bit installer
                new InstallInfoMsiNoLocation(
                    "https://github.com/adoptium/temurin25-binaries/releases/download/jdk-25%2B36/OpenJDK25U-jre_x64_windows_hotspot_25_36.msi",
                    HashAlgorithm.SHA256,
                    "eeae2fb19dc8d778a324d9b5c879f7afe9732487516807b75584436b01e0d8c3",
                    signature,
        ...
        {
            return ["openjdk-25-jre", "openjre-25", "openjdk-jre", "openjre", "jre"];
        }
        ...
        {
            logger.Info("Searching for newer version of Eclipse Temurin 25 JRE...");
            string json;
            using (var client = new HttpClient() { Timeout = TimeSpan.FromSeconds(25) })
            {
                try
                {
                    var task = client.GetStringAsync("https://api.adoptium.net/v3/assets/feature_releases/25/ga?heap_size=normal&image_type=jre&jvm_impl=hotspot&os=windows&page=0&page_size=1&project=jdk&sort_method=DEFAULT&sort_order=DESC&vendor=eclipse");
                    task.Wait();
                    json = task.Result;
                }
                catch (Exception ex)
                {
                    logger.Warn("Exception occurred while checking for newer version of Eclipse Temurin 25 JRE: " + ex.Message);
                    return null;
                }
            }
        ...
        public override List<Process> preUpdateProcess(DetectedSoftware detected)
        {
            ...
            {
                logger.Error("There is not enough information to uninstall the old Eclipse Temurin JRE 25 version.");
                return null;
            }
            ...
            if (!m.Success)
            {
                logger.Error("Could not extract GUID of old Eclipse Temurin JRE 25 version for pre-update process.");
                return null;
            }
